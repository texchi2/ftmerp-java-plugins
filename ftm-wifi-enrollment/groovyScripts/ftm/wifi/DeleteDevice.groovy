// DeleteDevice.groovy — permanently delete a device enrollment record
...
def deleteFtmDevice() {
    ...
    try {
        ...
        sql.execute("DELETE FROM enrolled_devices WHERE id = ?", [deviceId])
        return success([message: "Device '${existing.device_label}' (${existing.username}) permanently deleted"])
    } finally {
        sql.close()
    }
}

return deleteFtmDevice()
