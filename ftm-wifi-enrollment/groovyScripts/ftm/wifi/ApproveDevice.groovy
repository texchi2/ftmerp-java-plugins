...
def approveFtmDevice() {
    ...
    def approvedBy = context.userLogin?.getString("userLoginId") ?: "ofbiz-admin"
    ...
    try {
        ...
        if (existing.status != 'pending')
            return error("Device is not pending (current status: ${existing.status})")
        ...
        return success([message: "Device '${existing.device_label}' approved by ${approvedBy}"])
    } finally {
        sql.close()
    }
}

return approveFtmDevice()
